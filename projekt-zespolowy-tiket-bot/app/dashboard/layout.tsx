...
export default function GlobalDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Jeśli jesteśmy wewnątrz podkatalogu serwera [serverId], nie renderujemy tego globalnego layoutu
  // (Każdy serwer będzie miał tam swój własny, dedykowany layout)
    ...
    <div className="flex min-h-screen bg-[#101216] text-[#f2f3f5] antialiased">
      ...
      <aside className="w-64 border-r border-[#1e222b] bg-[#161920] p-6 flex flex-col gap-8 shrink-0">
        ...
        <div className="mt-auto pt-6 border-t border-[#1e222b]">
          <Link href={getPath('/')} className="text-sm font-medium text-[#6b7280] hover:text-white transition flex items-center gap-2">
            ← Strona główna bota
          </Link>
        </div>
      </aside>

      {/* KONTENER TREŚCI (Zawsze ciemny, wymuszony AMOLED mode) */}
